...
import { buildStellation, facePlanes, suggestDepth, formatCellsAtoms,
         parseCellsAny, selectedCells, extractMesh, atomKey } from '../lib/core.js';
...
for (const f of fresh) writeFileSync(join(DOCS, f.file), f.document);
...
${s.rules.map(r => `    <li>${esc(r)}</li>`).join('\n')}
  ...
  <p id="icCount"></p>
  <div id="icGrid" class="ic-grid"></div>
