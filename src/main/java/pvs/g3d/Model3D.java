...
public class Model3D {
    ...
    public static final int NORMAL=0,ANAGLYPH_RC = 1,ANAGLYPH_CR = 2, STEREO_PARALLEL=3, STEREO_CROSSED = 4;

    public Face		face[];				// array of faces
  
    public boolean	transformed;

    boolean gothead;
    public Matrix3D	mat;				// applied 3D transformation
    public double	
    ...
    */  
    public void paint( Graphics g, int displayType, boolean drawFaces, boolean drawLines ) {

            if(displayType == ANAGLYPH_RC || displayType == ANAGLYPH_CR){
                drawAnaglyph = true;
        ...
        for (int f = 0; f < nfaces; f++) {
            ...
            if(tnormals[i] != null && tnormals[i].z < 0 && drawFaces ) // it is backface
                continue;
