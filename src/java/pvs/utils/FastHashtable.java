...
/**
 * FastHashtable class. Maps keys to values. Any object can be used as
...
class FastHashtable extends Dictionary implements Cloneable {
  ...
  /**
   *  print statistic of current table 
   */   
    public void printStatistic(java.io.PrintStream out){
      int capacity = table.length;
      out.print("hashtable capacity: " + table.length + " count: " + count);
      int maxlength = 0;
      for (int i = capacity ; i-- > 0 ;) {
	int brlength = 0; 
	for (HashtableEntry old = table[i] ; old != null ; ) {
	  old = old.next;	  
	  brlength++;
	}
	if(brlength > maxlength ){
	  maxlength = brlength;
	}
      }      
	...
	if(brlength > 3 ){
      ...
      out.println(" maxlength: " + maxlength + " halfcount: " + halfcount + " n3:" + n3);
   }
