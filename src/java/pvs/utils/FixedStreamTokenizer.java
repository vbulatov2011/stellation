...
public class FixedStreamTokenizer {
    ...
    private FixedStreamTokenizer() {
	  ...
	  quoteChar('"');
	  quoteChar('\'');
 	  //parseNumbers(); // VB original tokenizer don't parse scientific notation correctly
          // so, we stop it from parsing numbers
     ...
     */
    public FixedStreamTokenizer(InputStream is) {
	    this();
        if (is == null) {
     ...
     * @since   JDK1.1
     */
    public FixedStreamTokenizer(Reader r) {
	    this();
        if (r == null) {
            throw new NullPointerException();
        }
