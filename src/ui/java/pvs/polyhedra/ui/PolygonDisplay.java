...
import pvs.polyhedra.PolygonArray;
...
import pvs.polyhedra.SubdivisionCurve;
import pvs.polyhedra.TransformNode;
import pvs.polyhedra.UseNode;
import pvs.polyhedra.Vector3D;
import pvs.polyhedra.stellation.ui.DlgPrint;
...
public class PolygonDisplay  implements PVSObserver {
  ...
  void makeUI(){
    ...
    WindowUtils.constrain(panel1,listPoly,    0,c++,1,1, GridBagConstraints.VERTICAL, GridBagConstraints.CENTER,1.,1.,2,2,2,2);   

    WindowUtils.constrain(frame,canvas,     0,0,1,1, GridBagConstraints.BOTH, GridBagConstraints.CENTER,1.,1.);
    WindowUtils.constrain(frame,panel1, 1,0,1,1, GridBagConstraints.VERTICAL, GridBagConstraints.CENTER,0.,1.);
  ...
  */
  class onEdit implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e){
      ...
      if(ind < 0 || ind >= polygons.size())
        return;
      showEditor( (PolygonDisplayNode)polygons.elementAt(ind) );
    }
  }
  ...
  static void showEditor( PolygonDisplayNode polygon )
  {
      if ( polygon instanceof PolygonArray) {
          new Dlg_PolygonArray((PolygonArray)polygon) .show();
          return;
      }
      if ( polygon instanceof SubdivisionCurve) {
          new Dlg_SubdivisionCurve((SubdivisionCurve)polygon) .show();
          return;
      }
      if ( polygon instanceof TransformNode) {
          new Dlg_TransformNode((TransformNode)polygon) .show();
          return;
      }
      if ( polygon instanceof PolygonParametrical) {
          new Dlg_PolygonParametrical((PolygonParametrical)polygon) .show();
          return;
      }
  }
  ...
  void parsePolygonFile(String fileName){
    ...
    try {
      ...
      FixedStreamTokenizer st = TransformNode.makeStreamTokenizer(r);
            
      PolygonDisplayParser .parseChildren( st, polygons, this );
...
class Dlg_SubdivisionCurve  /*implements PolygonEditor*/{
  ...
  void makeUI(){
    ...
    Panel panel1 = new Panel();
    panel1.setLayout(gbl);
    WindowUtils.constrain(panel1,curvePanel,                   0,0,1,1, GridBagConstraints.BOTH, GridBagConstraints.WEST,1.,1.);
    WindowUtils.constrain(panel1,curvePanel.getOptionsPanel(), 1,0,1,1, GridBagConstraints.NONE, GridBagConstraints.NORTH,0.,0.);

    Panel panel2 = new Panel();
    panel2.setLayout(gbl);
    
    WindowUtils.constrain(frame,panel1, 0,0,1,1, GridBagConstraints.BOTH, GridBagConstraints.CENTER,1.,1.);
    WindowUtils.constrain(frame,panel2, 0,1,1,1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER,1.,0.);
...
class Dlg_UseNode  /*implements PolygonEditor*/{
  ...
  void makeUI(){
    ...
    int c = 0;
    WindowUtils.constrain(panel1,tfReference, 1,c,1,1, GridBagConstraints.BOTH, GridBagConstraints.CENTER,1.,1.);
    WindowUtils.constrain(panel1,new Label("reference"), 0,c++,1,1, GridBagConstraints.NONE, GridBagConstraints.EAST,0.,1.);

    Panel panel3 = new Panel();    panel3.setLayout(gbl);
    WindowUtils.constrain(panel3,btnUpdate,      0,0,1,1, GridBagConstraints.NONE, GridBagConstraints.CENTER,1.,1.);    
    
    WindowUtils.constrain(frame,panel1, 0,0,1,1, GridBagConstraints.BOTH, GridBagConstraints.CENTER,1.,1.);
    WindowUtils.constrain(frame,panel3, 0,1,1,1, GridBagConstraints.NONE, GridBagConstraints.CENTER,1.,0.);
